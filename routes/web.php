...
Route::middleware('auth')->group(function () {
    ...
    // === CHECKLIST / SESSION EXECUTION ROUTES (Accessible by Housekeepers) ===
    ...
    Route::post('/sessions/{session}/rooms/{room}/tasks/{task}/photo', [ChecklistController::class, 'taskPhoto'])->name('checklist.task-photo');
    Route::post('/sessions/{session}/rooms/{room}/skip', [ChecklistController::class, 'skipRoom'])->name('checklist.skip-room');
    Route::post('/sessions/{session}/rooms/{room}/unskip', [ChecklistController::class, 'unskipRoom'])->name('checklist.unskip-room');

    // Property-level Checklist Actions
    Route::post('/sessions/{session}/property-tasks/{task}/toggle', [ChecklistController::class, 'togglePropertyTask'])->name('checklist.property-task.toggle');
    Route::post('/sessions/{session}/property-tasks/{task}/note', [ChecklistController::class, 'notePropertyTask'])->name('checklist.property-task.note');
    Route::post('/sessions/{session}/property-tasks/{task}/photo', [ChecklistController::class, 'propertyTaskPhoto'])->name('checklist.property-task.photo');
...
// === MANAGEMENT ROUTES (Restricted to Admin & Owner) ===
Route::middleware(['auth', 'role:admin|owner'])->group(function () {
    ...
    Route::prefix('properties')->name('properties.')->group(function () {
        ...
        Route::patch('{property}/rooms/order', [PropertyRoomOrderController::class, 'update'])->name('rooms.order');
        Route::post('{property}/rooms/attach', [PropertyRoomAttachController::class, 'store'])->name('rooms.attach');
    ...
    Route::get('/api/properties/{property}/assigned-rooms', [PropertyAssignmentsApiController::class, 'rooms'])->name('api.properties.assigned-rooms');
    Route::get('/api/properties/{property}/assigned-property-tasks', [PropertyAssignmentsApiController::class, 'propertyTasks'])->name('api.properties.assigned-property-tasks');

    // Rooms (Global)
    Route::resource('rooms', RoomController::class)->except(['show', 'create']);
    Route::post('/rooms/bulk-attach-tasks', [RoomController::class, 'bulkAttachTasks'])->name('rooms.bulk-attach-tasks');
    ...
    // Users (already protected, but merging here for clarity)
    ...
    Route::get('/rooms/suggest', [RoomSuggestionController::class, 'index'])->name('rooms.suggest');
    Route::get('/tasks/suggest', [TaskSuggestionController::class, 'index'])->name('tasks.suggest');
});
