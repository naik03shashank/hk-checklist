                    ...
                    <template x-for="media in existingMedia" :key="media.id">
                        <div class="relative group rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
                            ...
                            <div class="absolute top-0 right-0 p-1">
                                <form method="POST" :action="`/tasks/${taskId}/media/${media.id}`" @submit="confirm('Are you sure?') || $event.preventDefault()">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="bg-red-500 text-white rounded-full p-1 hover:bg-red-600 transition-colors shadow-sm">
