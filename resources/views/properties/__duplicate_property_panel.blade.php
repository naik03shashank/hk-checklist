@php
    ...
    $suffix = $suffix ?? '';
    $modalId = 'duplicate-property-' . $property->id . ($suffix ? '-' . $suffix : '');
    $formId = 'duplicate-property-form-' . $property->id . ($suffix ? '-' . $suffix : '');
@endphp

<x-preview-panel name="{{ $modalId }}" :overlay="true" side="right" initialWidth="34rem"
    ...
    <div class="p-4 h-full flex flex-col text-gray-800 dark:text-gray-100"
        x-data="duplicatePropertyPanel({
            initialName: @json($property->name),
            rooms: @js($roomsForJs),
            tasks: @js($tasksForJs),
    ...
    <x-slot:footer>
        <div class="flex items-center justify-end gap-2">
            <x-button variant="secondary"
                @click="$dispatch('close-preview-panel', '{{ $modalId }}')">
                Close
            </x-button>
            <x-button type="submit" form="{{ $formId }}"
