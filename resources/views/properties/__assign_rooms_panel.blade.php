@php
    $suffix = $suffix ?? '';
    $modalId = 'assign-rooms-' . $property->id . ($suffix ? '-' . $suffix : '');
    $eventId = 'assign-rooms-save-' . $property->id . ($suffix ? '-' . $suffix : '');
@endphp

<x-preview-panel name="{{ $modalId }}" :overlay="true" side="right" initialWidth="30rem"
    ...
    <x-slot:footer>
        <div class="flex items-center justify-end gap-2">
            <x-button variant="secondary" @click="$dispatch('close-preview-panel', '{{ $modalId }}')">
                Close
            </x-button>

            <x-button type="button" class="bg-indigo-600 hover:bg-indigo-700"
                @click="window.dispatchEvent(new CustomEvent('{{ $eventId }}'))">
                Save selection
            </x-button>
        </div>
    </x-slot:footer>
