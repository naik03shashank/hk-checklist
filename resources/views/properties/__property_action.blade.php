<x-action-dropdown align="right" width="w-56" label="Property actions">
    ...
    @role('admin|owner')
        <x-dropdown.item as="button"
            x-on:click="
            $dispatch('open-preview-panel', 'duplicate-property-{{ $property->id }}{{ isset($suffix) ? '-' . $suffix : '' }}');
        ...
        <x-dropdown.item as="button"
            x-on:click="
            $dispatch('open-preview-panel', 'assign-rooms-{{ $property->id }}{{ isset($suffix) ? '-' . $suffix : '' }}');
            $dispatch('dropdown-close');
         ">
            {{-- inline plus icon --}}
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 24 24" fill="currentColor">
    ...
    @role('admin|owner')
        ...
        <x-dropdown.item as="button"
            ...
            x-on:click="
                    $dispatch('open-modal', 'confirm-delete-property-{{ $property->id }}{{ isset($suffix) ? '-' . $suffix : '' }}');
                    $dispatch('dropdown-close');
                ">
            {{-- trash icon --}}
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 24 24" fill="currentColor">
...
<x-modal name="confirm-delete-property-{{ $property->id }}{{ isset($suffix) ? '-' . $suffix : '' }}" :show="false" maxWidth="md">
