...
export default function checklist(config = {}) {
    return {
        ...
        init() {
            ...
            // Intercept all checklist form submissions - only once
            if (!window.checklistListenersAttached) {
                this.setupFormHandlers();
                window.checklistListenersAttached = true;
            }
        },

        setupFormHandlers() {
            // Use named functions for toggle/note to avoid duplication if called again
            // though the init() check should handle it.

            // Handle toggle buttons - use event delegation
            const handleToggleDelegate = (e) => {
            ...
            document.addEventListener('click', handleToggleDelegate);

            // Handle note saves - use event delegation
            const handleNoteSaveDelegate = (e) => {
            ...
            document.addEventListener('click', handleNoteSaveDelegate);

            // Handle photo uploads - use event delegation
            const handlePhotoUploadDelegate = (e) => {
                const form = e.target;
            ...
            document.addEventListener('submit', handlePhotoUploadDelegate);
        },
