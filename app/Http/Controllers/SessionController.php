...
class SessionController extends Controller
{
    ...
    private function prepareSessionData(CleaningSession $session): array
    {
        ...
        $isHousekeeper = auth()->user()->hasRole('housekeeper') && !auth()->user()->hasAnyRole(['admin', 'owner']);

        $rooms = $session->property->rooms()
            ->with([
                'tasks' => function($q) use ($isHousekeeper) {
                    $q->orderBy('room_task.sort_order')->orderBy('tasks.name');
                    if ($isHousekeeper) {
                        $q->where('room_task.visible_to_housekeeper', true);
                    }
                },
                'tasks.media',
            ])
        ...
        // Ensure mandatory "8 Photos" task exists
        $mandatoryPhotoTask = \App\Models\Task::firstOrCreate(
            ['name' => '📸 Take 8 Photos of this Room (Required)'],
            ['type' => 'room', 'is_default' => false, 'instructions' => 'Please take at least 8 high-quality photos of this room to document its condition.']
        );

        // Ensure existing checklist items
        $existingItems = $session->checklistItems->groupBy(fn($i) => $i->room_id . '-' . $i->task_id);
        $newItems = [];
        $now = now();
        $uid = auth()->id();

        foreach ($rooms as $room) {
            // Check if mandatory task is already attached, if not attach it
            if (!$room->tasks->contains($mandatoryPhotoTask->id)) {
                $room->tasks()->attach($mandatoryPhotoTask->id, ['sort_order' => 0]);
                // We don't necessarily need to reload here if we handle the mandatory task manually in the loop below
            }
            ...
            // Explicitly handle mandatory task if it wasn't in $room->tasks yet
            $mandatoryKey = $room->id . '-' . $mandatoryPhotoTask->id;
            if (!$existingItems->has($mandatoryKey) && !$room->tasks->contains($mandatoryPhotoTask->id)) {
                 $newItems[] = [
                    'session_id' => $session->id,
                    'room_id'    => $room->id,
                    'task_id'    => $mandatoryPhotoTask->id,
                    'user_id'    => $uid,
                    'checked'    => false,
                    'created_at' => $now,
                    'updated_at' => $now,
        ...
        if (!empty($newItems)) {
            \App\Models\ChecklistItem::insert($newItems);
            $session->load('checklistItems.photos'); // Reload with new items
        }
        ...
        $pendingInventory = $allInventoryTasksCount - $checkedInventoryTasksCount;

        if ($session->status === 'completed') {
             $stage = 'summary';
        } elseif ($pendingPreCleaning > 0) {
             $stage = 'pre_cleaning';
             ...
             $stage = 'post_cleaning';
        } elseif ($pendingInventory > 0) {
             $stage = 'inventory';
        } else {
             // If everything else is done, we might show a final review or photos stage
             // But since photos are now tasks, this stage is minimal or just for review
             $stage = 'photos'; 
        }
